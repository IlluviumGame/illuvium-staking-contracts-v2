...
async function main(): Promise<void> {
  ...
  // Parse test addresses info
  const testData = JSON.parse(fs.readFileSync(__dirname + "/data/test_addresses_goerli.json", "utf-8"));
  testData.forEach((testDataInstance: any) => {
    treeData.push({
      account: testDataInstance.account,
      pendingV1Rewards: ethers.BigNumber.from(testDataInstance.pendingV1Rewards),
      weight: ethers.BigNumber.from(testDataInstance.weight),
    });
  });
  /*
  // Parse mainnet addresses
  for (let i = 76; i <= 20316; i++) {
    treeData[i] = {
  ...
  */
  ...
  fs.writeFileSync("./scripts/data/goerli_tree.json", JSON.stringify(treeData));
