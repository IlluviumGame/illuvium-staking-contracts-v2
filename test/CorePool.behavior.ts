...
export function upgradePools(): () => void {
  return function () {
    ...
    it("should revert upgrading ilv pool from invalid admin", async function () {
      ...
      await expect(this.ilvPool.connect(this.signers.alice).upgradeTo(implementationAddress as string)).reverted;
    });
    it("should revert upgrading lp pool from invalid admin", async function () {
      ...
      await expect(this.lpPool.connect(this.signers.alice).upgradeTo(implementationAddress as string)).reverted;
    });
  };
}
