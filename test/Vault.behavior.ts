  ...
  return function () {
    it("should set core pools correctly", async function () {
      ...
      await this.vault
        ...
        .setCorePools(ilvPoolAddress, lpPoolAddress, lockedPoolV1MockedAddress);

      const { ilvPool, pairPool, lockedPoolV1 } = await this.vault.pools();
    ...
    it("should revert if ilvPool = address(0)", async function () {
      ...
      await expect(
        this.vault.connect(this.signers.deployer).setCorePools(AddressZero, lpPoolAddress, lockedPoolV1MockedAddress),
      ).reverted;
    });
    ...
    it("should revert if pairPool = address(0)", async function () {
      ...
      await expect(
        this.vault.connect(this.signers.deployer).setCorePools(ilvPoolAddress, AddressZero, lockedPoolV1MockedAddress),
      ).reverted;
    });
    ...
    it("should revert if lockedPoolV1 = address(0)", async function () {
      ...
      await expect(this.vault.connect(this.signers.deployer).setCorePools(ilvPoolAddress, lpPoolAddress, AddressZero))
        .reverted;
    });
